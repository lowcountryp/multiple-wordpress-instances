...
/************************************************************************************************/
/* 7CARE - Get the unencrypted password.  It will be used to populate the fields on the purchased instance  */ 
/************************************************************************************************/				
	...
	/**
	 * The customer has placed the order, now build the site directory structure and db, then hand off and create the subscriber admin user record ...
	 * ==> 7Care
	 * ------------
	 * ------------
	 */	
function _7care_build_provision( $order_id, $instanceID, $pass7c ) {

/* NOTE: Woocommerce $order_id is really the WP POST ID, not the order#... */
$ord = $order_id;
/* instance should be blank, coming in - populated by mySQL for handing back to Woo Commerce */
$instance = $instanceID;

$host = "localhost";
$user = "root";
$password = "#ululati0n";
$database = "aca1304506453803";
/* Include the customer user field - it's the WP userID number and is passed to provisioning function, along with first and last name... */

$result0 = "select meta_key, meta_value from wp_postmeta where post_id = '$ord' and meta_key in ('_billing_email', '_order_key', '_billing_phone', 'Customer IP Address','_billing_last_name','_billing_first_name', '_customer_user') order by meta_key;";
...
if (!$result = mysql_query($result0)){
die(mysql_error());
};

while($row = mysql_Fetch_array($result)){
	$thisKy = $row['meta_key'];
	$thisVa = $row['meta_value'];
	switch ($thisKy) {
		case 'Customer IP Address':
			$custIP=$thisVa;
		break;
		case '_billing_email':
			$custID=$thisVa;
		...
		case '_billing_first_name':
			$customerFirst=$thisVa;
		break;
		case '_customer_user':
			$customerUserNbr=$thisVa;
		break;
	}
}

$result_GetUserInfo = "select um.meta_key, um.meta_value, u.user_login, u.user_pass, u.user_nicename, u.user_email, u.display_name from wp_usermeta um, wp_users u where um.user_id = u.id and um.user_id = '$customerUserNbr';";

$resultX = mysql_query($result_GetUserInfo);
if($resultX === FALSE) {
...
while($row = mysql_Fetch_array($resultX)){
	$thisKy = $row['meta_key'];
	$thisVa = $row['meta_value'];
	$thisULogin = $row['user_login'];
	$thisUpass = $row['user_pass'];
	$thisUNice = $row['user_nicename'];
	$thisEmailAddr = $row['user_email'];
	$thisUdispName = $row['display_name'];
	
/* These values from wp_users are echoed on all the returned records, so populate them on the first case */
	switch ($thisKy) {
		case 'first_name': 
			$thisFirstName=$thisVa;
			$custLogin=$thisULogin;
			$custPass=$thisUpass;// not used - use the pw trapped in order checkout, instead (pass7c)
			$custNice=$thisUNice;
			$custDispNm=$thisUdispName;
		break;
		case 'last_name':
			$thisLastName=$thisVa;
		break;
...
$database2 = "mst7care";
mysql_select_db($database2,$connection) or die("Error in selecting the database:".mysql_error());

		if (! $resGrabSID = mysql_query("select 7CareSeq, 7CareSeqID, buildDt, assignedDt, activateDt,custID, isAssigned, status, orderNum, billingPhone from SIDMaster where isAssigned = ' ' for update;")) {
		die(mysql_error());
		};

$row = mysql_fetch_row($resGrabSID);
		$v0 = $row[0]; //seq
		$v1 = $row[1]; //sid id
		$v2 = $row[2]; //build dt
		$v3 = $row[3]; //assigned dt
		$v4 = $row[4]; //activate dt
		$v5 = $row[5]; //custID
		$v6 = $row[6]; //isAssigned
		$v7 = $row[7]; //status
		$v8 = $row[8]; //order number
		$v9 = $row[9]; //billing phone

/* Trap the instance ID for passing back to Woo Commerce... */
$instanceID = $v1;
$grabSIDupdateString = "update SIDMaster set buildDt = CURDATE(), assignedDt = CURDATE(), activateDt = CURDATE(), custID = '$custID', isAssigned = '1', status = '1', orderNum = '$orderNum', billingPhone = '$custBillingPh' where 7CareSeq = '$v0';";

	if (! $updateRes = mysql_query($grabSIDupdateString)) {
	echo mysql_error();
	} else
	{
/*###Execute the script to create the instance passing: dbname dbuser dbpass queuename adminEmailName */
$dbuser = "u$v1";
$queue = "q9";
$pass = "obscur3";
$thisUserLevel = "gzy34adm";

/* Hand the parms off to the shell to build out the directory and db structure ... */
$sidBuild = exec("LD_LIBRARY_PATH=path_to_shared_lib:$LD_LIBRARY_PATH && export LD_LIBRARY_PATH && /var/www/build7c_3.sh $v1 $dbuser $pass $queue $custID");

/* Granting authority  - an extracted, specialized 'grant authority' shell script, otherwise what follows will fail on authority ... */
$sidGrant = exec("LD_LIBRARY_PATH=path_to_shared_lib:$LD_LIBRARY_PATH && export LD_LIBRARY_PATH && /var/www/_7Cgrant1.sh $v1 $dbuser $pass");

/* The call to provisioning. This will build out the customer's SID enough for the subscriber/admin to login.  */
/* From there, they will use the REGISTER function to add their 7Care group members*/
$this->_7care_RemoteInstance_SubscriberUserSetup($order_id, $instanceID, $thisCred, $thisFirstName, $thisLastName, $thisEmailAddr, $pass7c, $thisUserLevel );
//$sidSubscriberUserSetup = exec("LD_LIBRARY_PATH=path_to_shared_lib:$LD_LIBRARY_PATH && export LD_LIBRARY_PATH && /var/www/_7Csubscribersetup.sh $order_id, $instanceID, $thisCred, $thisFirstName, $thisLastName, $thisEmailAddr, $custPass, $thisUserLevel");
	};

return array  ($ord, $instanceID );

}
/******************************************/
/* END BUILD/PROVISION FUNCTION FOR 7CARE */
/******************************************/

/*******************************************************************************/	
/* BUILD/PROVISION REMOTE INSTANCE USER SETUP FUNCTION FOR 7CARE               */ 
/* The customer has placed the order, the directory and DB are built, now      */
/* populate the WP with enough user information for the subscriber to login    */
/* Use the information provided during order entry to create                   */ 
/* the 7Care subscriber/admin user on the instance they just purchased...      */
/*******************************************************************************/
...
/* 7Care architecture call for multiple 'queues', each populated with multiple instances.  At launch, there is only one queue, but building the queue handling will need to be done soon...*/
$queue = 'q9';
...
$connUserTbl = $instanceID . '.wp_users';
$connUserMetaTbl = $instanceID . '.wp_usermeta';
$thisUserRegistered = gmdate('Y-m-d H:i:s');
...
$connection2 = mysql_pconnect($host2,$user2,$password2, true) or die("Could not connect: ".mysql_error());
	
mysql_select_db($database2,$connection2) or die("Error in selecting the database:".mysql_error());
/* Create a new user in the remote instance wp_users file... */
$resultAddUser = mysql_query("INSERT INTO $connUserTbl(user_login, user_email, user_nicename, user_registered, display_name) VALUES ('$thisFirstName', '$thisEmailAddr', '$thisFirstName', '$thisUserRegistered', '$thisFirstName')", $connection2);
		if($resultAddUser === FALSE) {
			die(mysql_error()); // TODO: better error handling
		}

/* Retrieve the just-created ID number that was generated... */
$fetchThatUser = mysql_query("select ID from $connUserTbl where user_login = '$thisFirstName'", $connection2);
		if($fetchThatUser === FALSE) {
			die(mysql_error()); // TODO: better error handling
		}
$fetchThatID = mysql_fetch_row($fetchThatUser);

/* This is the new ID: */
$ID_work = $fetchThatID[0];

/* Make the subscribers password the same as their 7Care password... */
$newPassword = mysql_query("UPDATE $connUserTbl SET user_pass = MD5('$thisUpass') WHERE ID = '$ID_work'", $connection2) or trigger_error(mysql_error()." ".$newPassword);

/* We're creating a user from copy in wp_usermeta, so fetch Ricky Bobby's USERID (21) number with gzy34adm role, and overwrite it... */
$_mMETA0 = mysql_query("update $connUserMetaTbl set user_id = '$ID_work' where user_id = 21;", $connection2);
		if($_mMETA0 === FALSE) {
			die("Error in META0 ".mysql_error()); // TODO: better error handling
		}

$fetchMETA0 = $_mMETA0[0];

$_mMETA1 = mysql_query("update $connUserMetaTbl set meta_value = '$thisFirstName' where meta_key = 'nickname' and user_id = '$ID_work';", $connection2);
		if($_mMETA0 === FALSE) {
			die("Error in META1 ".mysql_error()); // TODO: better error handling
		}
$_mMETA2 = mysql_query("update $connUserMetaTbl set meta_value = '$thisFirstName' where meta_key = 'first_name' and user_id = '$ID_work';", $connection2);
		if($_mMETA0 === FALSE) {
			die("Error in META2 ".mysql_error()); // TODO: better error handling
		}
$_mMETA3 = mysql_query("update $connUserMetaTbl set meta_value = '$thisLastName' where meta_key = 'last_name' and user_id = '$ID_work';", $connection2);
		if($_mMETA0 === FALSE) {
			die("Error in META3 ".mysql_error()); // TODO: better error handling
		}
$_mMETA4 = mysql_query("update $connUserMetaTbl set meta_value = '$thisEmailAddr' where meta_key = 'user_email' and user_id = '$ID_work';", $connection2);
		if($_mMETA0 === FALSE) {
			die("Error in META4 ".mysql_error()); // TODO: better error handling
		}
// Now, delete Ricky Bobby's old gzy34adm information in wp_users...
$_mMETA5 = mysql_query("delete from $connUserTbl where user_login = 'cal' and ID = 21;", $connection2);
		if($_mMETA0 === FALSE) {
			die("Error in META5 ".mysql_error()); // TODO: better error handling
		}
}
...
/************************************************************************************************/
/* 7CARE - Get the password.  It will be used to populate the fields on the purchased instance  */ 
/************************************************************************************************/				
				
			$pass7c = $this->posted['account_password'];
			...
			try {
				...
				if ( $woocommerce->cart->needs_payment() ) {
					...
					if ( $result['result'] == 'success' ) {
						//7Care hooks to build the instance - START:
						$instance = 'SID';
						list ($order_id, $instance) = $this->_7care_build_provision($order_id, $instance, $pass7c);
						//echo "AFTER BUILD INSTANCE ID IS: $instance \n";
						//7Care hooks to build the instance - END	
